...
int main() {
  ...
  while (true) {
    ...
    Pool pool(root);
    auto tmp1 = pool.forward(session, input);
    assignedI = Assign(root.WithOpName("I_assign2"), input, tmp1[0]);
    LOGs(outputs[0].shape());

    Activation act(root);
    auto tmp2 = act.forward(session, input);
    assignedI = Assign(root.WithOpName("I_assign3"), input, tmp2[0]);
    LOGs(outputs[0].shape());

    FC fc(root);
    auto tmp3 = fc.forward(session, input);
    assignedI = Assign(root.WithOpName("I_assign4"), input, tmp3[0]);
    LOGs(outputs[0].shape());
